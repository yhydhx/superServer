...
void createsem(int *sid, key_t key, int member);
void locksem(int std, int member);
...
void createsem(int *sid, key_t key, int members){
	...
	if(members > SEMMSL){
		printf("Sorry, max number of semaphore s in a set is %d\n", SEMMSL );
		exit(1);
	}
	...
	//Iinitialize all members (could be done with SETALL)
...
unsigned short get_member_count(int sid){
	...
	//return the number of  members in the semaphore ser
